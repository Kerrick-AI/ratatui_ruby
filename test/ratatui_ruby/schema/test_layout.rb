...
class TestLayout < Minitest::Test
  ...
  def test_layout_creation
    ...
    l = RatatuiRuby::Layout::Layout.new(direction: :vertical, constraints: [RatatuiRuby::Layout::Constraint.percentage(100)], children: [p])
  ...
  def test_layout_with_flex
    l = RatatuiRuby::Layout::Layout.new(flex: :space_between)
    assert_equal :space_between, l.flex

    l2 = RatatuiRuby::Layout::Layout.new(flex: :center)
    ...
    with_test_terminal(20, 3) do
      l = RatatuiRuby::Layout::Layout.new(
        ...
        children: [
          ...
          RatatuiRuby::Widgets::Paragraph.new(text: "C"),
        ]
      )
  ...
  def test_flex_modes_constant
    assert_includes RatatuiRuby::Layout::Layout::FLEX_MODES, :legacy
    assert_includes RatatuiRuby::Layout::Layout::FLEX_MODES, :start
    assert_includes RatatuiRuby::Layout::Layout::FLEX_MODES, :center
    assert_includes RatatuiRuby::Layout::Layout::FLEX_MODES, :end
    assert_includes RatatuiRuby::Layout::Layout::FLEX_MODES, :space_between
    assert_includes RatatuiRuby::Layout::Layout::FLEX_MODES, :space_around
  ...
  def test_nested_layout
    ...
    outer = RatatuiRuby::Layout::Layout.new(direction: :vertical, children: [inner])
  ...
  def test_render
    with_test_terminal(20, 10) do
      l = RatatuiRuby::Layout::Layout.new(
        ...
        children: [
          RatatuiRuby::Widgets::Paragraph.new(text: "Top"),
          RatatuiRuby::Widgets::Paragraph.new(text: "Bottom"),
        ]
      )
      RatatuiRuby.draw { |f| f.render_widget(l, f.area) }
  ...
  def test_render_flex_center
    with_test_terminal(20, 3) do
      l = RatatuiRuby::Layout::Layout.new(
        direction: :horizontal,
        flex: :center,
        constraints: [RatatuiRuby::Layout::Constraint.length(6)],
        children: [RatatuiRuby::Widgets::Paragraph.new(text: "Center")]
      )
      RatatuiRuby.draw { |f| f.render_widget(l, f.area) }
  ...
  def test_render_flex_end
    with_test_terminal(20, 3) do
      l = RatatuiRuby::Layout::Layout.new(
        direction: :horizontal,
        flex: :end,
        constraints: [RatatuiRuby::Layout::Constraint.length(5)],
        children: [RatatuiRuby::Widgets::Paragraph.new(text: "Right")]
      )
      RatatuiRuby.draw { |f| f.render_widget(l, f.area) }
  ...
  def test_split_vertical
    ...
    rects = RatatuiRuby::Layout::Layout.split(
      ...
      direction: :vertical,
      constraints: [
        RatatuiRuby::Layout::Constraint.length(5),
  ...
  def test_split_returns_rect_objects
    area = RatatuiRuby::Layout::Rect.new(x: 10, y: 5, width: 60, height: 20)
    rects = RatatuiRuby::Layout::Layout.split(
      area,
      direction: :horizontal,
      constraints: [RatatuiRuby::Layout::Constraint.percentage(100)]
    )
    assert_instance_of RatatuiRuby::Layout::Rect, rects.first
  ...
  def test_split_with_flex
    ...
    rects = RatatuiRuby::Layout::Layout.split(
      area,
      direction: :horizontal,
      constraints: [RatatuiRuby::Layout::Constraint.length(20)],
      flex: :center
    )
  ...
  def test_split_flex_modes_comprehensive
    area = RatatuiRuby::Layout::Rect.new(x: 0, y: 0, width: 100, height: 10)
    constraints = [RatatuiRuby::Layout::Constraint.length(10), RatatuiRuby::Layout::Constraint.length(10)]

    # :start
    rects = RatatuiRuby::Layout::Layout.split(area, direction: :horizontal, constraints:, flex: :start)
    assert_equal 0, rects[0].x
    assert_equal 10, rects[1].x

    # :end
    rects = RatatuiRuby::Layout::Layout.split(area, direction: :horizontal, constraints:, flex: :end)
    assert_equal 80, rects[0].x
    assert_equal 90, rects[1].x

    # :space_between (0, 90)
    rects = RatatuiRuby::Layout::Layout.split(area, direction: :horizontal, constraints:, flex: :space_between)
    assert_equal 0, rects[0].x
    assert_equal 90, rects[1].x
    ...
    rects = RatatuiRuby::Layout::Layout.split(area, direction: :horizontal, constraints:, flex: :space_around)
    assert rects[0].x > 0
    assert rects[1].x > rects[0].x + 10
    assert rects[1].x < 90

    # :space_evenly
    rects = RatatuiRuby::Layout::Layout.split(area, direction: :horizontal, constraints:, flex: :space_evenly)
    assert rects[0].x > 0
    assert rects[1].x > rects[0].x + 10
  end
  ...
  def test_split_complex_constraints
    ...
    constraints = [
      ...
      RatatuiRuby::Layout::Constraint.max(20),
      RatatuiRuby::Layout::Constraint.percentage(50),
      RatatuiRuby::Layout::Constraint.min(5),
    ]
    # layout logic will try to satisfy these.
    rects = RatatuiRuby::Layout::Layout.split(area, direction: :vertical, constraints:)
